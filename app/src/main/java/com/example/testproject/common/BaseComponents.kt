...
//        intentsDisposable = Observable.merge( viewIntents(viewIntentsConsumer))
        intentsDisposable = viewIntents(viewIntentsConsumer)
            .scan(initState()) {
                    previousState, stateChanges -> reduceState(previousState, stateChanges)
            }
